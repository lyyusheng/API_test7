...
class TestCases(unittest.TestCase):
    ...
    def test_cases(self, case):
        ...
        # 替换loan_id,使用正则还可以替换mobilephone，pwd
        # 老方法：
        # if case['Params'].find('loanid') != -1:
        #     param = eval(case['Params'].replace('loanid', str(
        #         getattr(GetData, 'LOAN_ID'))))  # 因为拿到的数据是int类型 replace只能用在字符串之间的替换 所以用str强转一下
        # else:
        ...
        # 判断是否要查询数据库
        if case['sql'] is not None:  # 如果sql语句不为None 那就是要进行数据库的查询操作
            loan_id = DoMysql().do_pymysql(eval(case['sql'])['sql'], 1)  # 返回的是元组，所以我们存储数据的时候 最好是根据索引拿到值之后 再去做进一步操作
            setattr(GetData, 'loanid', str(loan_id[0]))  # loanid与Excel中的保持一致。从数据库查到的loan_id是int， 因为要用到正则表达式替换，所以改成str
