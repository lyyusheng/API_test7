...
from FuXi2.my_log import MyLog  #引入日志模块的Mylog类
...
class DoExcel():
    ...
    def read_case(self):
        '''读取指定表单的数据，以嵌套列表的形式存储，每一行都是一个子列表'''
    ...
    def write_case(self,row,col,value):
        ...
        sheet=wb[self.sheet_name]    #定位表单
        sheet.cell(row,col).value=value #定位单元格，或者第二种方法sheet.cell(row,col,value)
